...
namespace Nut {
	...
	OpenGLShader::OpenGLShader(const std::string& path) : m_FilePath(path)
	{
		...
		{
			// Measuaring shader compilation duration
			Timer timer;
			CompileOrGetVulkanBinaries(processedShaderSourceMap);
			CompileOrGetOpenGLBinaries();
			CreateProgram();
			NT_CORE_WARN("Shader creation took {0} ms", timer.ElapsedMillis());
		}
	...
	std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string& source)
	{
		...
		while (pos != std::string::npos)
		{
			...
			NT_CORE_ASSERT(Utils::ShaderTypeFromString(type), "Invalid shader type");
	...
	void OpenGLShader::CompileOrGetVulkanBinaries(const std::unordered_map<GLenum, std::string>& shaderSources)
	{
		...
		// Toggle shaderc optimisation, maybe disable for debugging?
		...
		for (auto&& [stage, source] : shaderSources)
		{
			...
			else
			{
				...
				{
					NT_CORE_ERROR(module.GetErrorMessage());
					NT_CORE_ASSERT(false, "Shader compilation failed");
				}
	...
	void OpenGLShader::CreateProgram()
	{
		...
		if (isLinked == GL_FALSE)
		{
			...
			NT_CORE_ERROR("Shader linking failed ({0}):\n{1}", m_FilePath, infoLog.data());
	...
	void OpenGLShader::Reflect(GLenum stage, const std::vector<uint32_t>& shaderData)
	{
		...
		NT_CORE_TRACE("OpenGLShader::Reflect - {0} {1}", Utils::GLShaderStageToString(stage), m_FilePath);
		NT_CORE_TRACE("    {0} uniform buffers", resources.uniform_buffers.size());
		NT_CORE_TRACE("    {0} resources", resources.sampled_images.size());

		NT_CORE_TRACE("Uniform buffers:");
		for (const auto& resource : resources.uniform_buffers)
		{
			...
			NT_CORE_TRACE("  {0}", resource.name);
			NT_CORE_TRACE("    Size = {0}", bufferSize);
			NT_CORE_TRACE("    Binding = {0}", binding);
			NT_CORE_TRACE("    Members = {0}", memberCount);
		}
	}
